...
const createAndroidPushPayload = function (data) {
	console.log('createAndroidPushPayload Invoked')
	let set = {};
	let fieldsToFill = ["type", "title", "message", "priority", "sound", "image", "contentType", "category", "click_action", "category", "activityId", "userId", "activityName", "activityType", "notesId", "senderId", "receiverId", "requestId", "senderUserName", "senderUserProfilePic", "senderUserType", "rewardType","publisherName","agoraChannelName","uid","expirationTimeInSeconds","role","account","subscriberId","isAudio","tokenId","subscriberName"];
...
const createIOSPushPayload = function (data) {
	console.log('createIOSPushPayload Invoked')
	let set = {};
	let fieldsToFill = ["type", "title", "message", "body", "mutableContent", "threadId", "priority", "sound", "image", "contentType", "category", "activityName", "activityType", "notesId", "category", "activityId", "userId", "senderId", "receiverId", "requestId", "senderUserName", "senderUserProfilePic", "senderUserType", "rewardType","publisherName","agoraChannelName","uid","expirationTimeInSeconds","role","account","subscriberId","isAudio","tokenId","subscriberName"];
...
const createWebPushPayload = function (data) {
	console.log('createWebPushPayload Invoked')
	let set: any = {};
	let fieldsToFill = ["type", "title", "message", "body", "mutableContent", "threadId", "priority", "sound", "image", "contentType", "category", "activityName", "activityType", "notesId", "category", "activityId", "userId", "senderId", "receiverId", "requestId", "senderUserName", "senderUserProfilePic", "senderUserType", "rewardType","publisherName","agoraChannelName","uid","expirationTimeInSeconds","role","account","subscriberId","isAudio","tokenId","subscriberName"];
...
const convertStringTimeToTimestamp = function (value: string) {
	...
	// let hours = Number(value.split(" ")[0].split(":")[0]);
	// let minutes = Number(value.split(" ")[0].split(":")[1]);

	// return time === "PM" ? ((hours + 12) * 60 * 60 * 1000) + (minutes * 60 * 1000) : (hours * 60 * 60 * 1000) + (minutes * 60 * 1000);
...
const encryptData = (text: string) => {
	try {
		// const cip = createCipher("aes192", SERVER.ENC);
		...
		switch (request.method.toUpperCase()) {
			...
			case "PUT":
				if (request.payload && request.payload.data) {
					// const decipher = createDecipher("aes192", SERVER.ENC);
					// let decrypted = decipher.update(request.payload.data, "hex", "utf8");
...
const getValidMediaType = (mediaUrl: string) => {
	try {
		...
		// var re = /(?:\.([^.]+))?$/;
		// var ext = re.exec(mediaExtension)[1];
		// var ext = ext.split("?")[0];
